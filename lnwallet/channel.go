...
func (lc *LightningChannel) diskHtlcToPayDesc(feeRate dcrutil.Amount,
	...
	var (
		ourP2WSH, theirP2WSH                 []byte // TODO(davec): P2SH
...
func (lc *LightningChannel) extractPayDescs(commitHeight uint64,
	isPendingCommit bool, feeRate dcrutil.Amount,
...
func (lc *LightningChannel) diskCommitToMemCommit(isLocal, isPendingCommit bool,
	...
	localCommitPoint, remoteCommitPoint *secp256k1.PublicKey) (*commitment, error) {
...
type LightningChannel struct {
	// netParams is the set of parameters that the channel operates on.
	...
	pendingFeeUpdate *dcrutil.Amount
	...
	// initiated.
	pendingAckFeeUpdate *dcrutil.Amount
	...
	fundingP2WSH []byte // TODO(davec): P2SH
...
func NewLightningChannel(signer Signer, events chainntnfs.ChainNotifier,
	fe FeeEstimator, state *channeldb.OpenChannel, netParams *chaincfg.Params) (*LightningChannel, error) {
	...
	fundingPkScript, err := witnessScriptHash(multiSigScript) // TODO(davec): payToScriptHash?
	if err != nil {
		return nil, err
	}
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	feeRate dcrutil.Amount, remoteCommitKeys *commitmentKeyRing,
	...
	case *lnwire.UpdateAddHTLC:
		...
		if !isDustRemote {
			theirP2WSH, theirWitnessScript, err := genHtlcScript( // TODO(davec): P2SH
...
func (lc *LightningChannel) restoreStateLogs(
	...
	for _, logUpdate := range pendingRemoteCommitDiff.LogUpdates {
		...
		if feeUpdate, ok := logUpdate.UpdateMsg.(*lnwire.UpdateFee); ok {
			newFeeRate := dcrutil.Amount(feeUpdate.FeePerKw)
...
func newBreachRetribution(chanState *channeldb.OpenChannel, stateNum uint64,
	...
	remoteWitnessHash, err := witnessScriptHash(remotePkScript) // TODO(davec): should be a p2sh
	if err != nil {
		return nil, err
	}
...
func htlcTimeoutFee(feePerKw dcrutil.Amount) dcrutil.Amount {
	return (feePerKw * HtlcTimeoutWeight) / 1000
}

// htlcSuccessFee returns the fee in satoshis required for an HTLC success
// transaction based on the current fee rate.
func htlcSuccessFee(feePerKw dcrutil.Amount) dcrutil.Amount {
...
func htlcIsDust(incoming, ourCommit bool,
	feePerKw, htlcAmt, dustLimit dcrutil.Amount) bool {
...
func (lc *LightningChannel) createCommitmentTx(c *commitment,
	...
	commitFee := dcrutil.Amount((int64(c.feePerKw) * totalCommitWeight) / 1000)
...
func genRemoteHtlcSigJobs(keyRing *commitmentKeyRing,
	...
	feePerKw := remoteCommitView.feePerKw // TODO(davec): feePerKb?
...
func (lc *LightningChannel) createCommitDiff(
	newCommit *commitment, commitSig *secp256k1.Signature,
	htlcSigs []*secp256k1.Signature) (*channeldb.CommitDiff, error) {
...
func (lc *LightningChannel) SignNextCommitment() (*secp256k1.Signature, []*secp256k1.Signature, error) {
	lc.Lock()
	defer lc.Unlock()
	...
	htlcSigs := make([]*secp256k1.Signature, 0, len(sigBatch))
...
func genHtlcSigValidationJobs(localCommitmentView *commitment,
	keyRing *commitmentKeyRing, htlcSigs []*secp256k1.Signature,
...
// state.
func (lc *LightningChannel) ReceiveNewCommitment(commitSig *secp256k1.Signature,
	htlcSigs []*secp256k1.Signature) error {
...
func (lc *LightningChannel) AddHTLC(htlc *lnwire.UpdateAddHTLC) (uint64, error) {
	...
	if !htlcIsDust && lc.channelState.IsInitiator {
		htlcFee := lnwire.NewMSatFromSatoshis(
			dcrutil.Amount((int64(feePerKw) * HtlcWeight) / 1000),
		)
...
func newHtlcResolution(signer Signer, localChanCfg *channeldb.ChannelConfig,
	...
	feePewKw, dustLimit dcrutil.Amount, csvDelay uint32,
...
func extractHtlcResolutions(feePerKw dcrutil.Amount, ourCommit bool,
...
func (lc *LightningChannel) CompleteCooperativeClose(localSig, remoteSig []byte,
	...
	// TODO(davec): script version constant or part of tx?
	//
...
func (lc *LightningChannel) availableBalance() (lnwire.MilliSatoshi, int64) {
	...
	if lc.channelState.IsInitiator {
		additionalFee := lnwire.NewMSatFromSatoshis(
			dcrutil.Amount((int64(feePerKw) * totalCommitWeight) / 1000),
		)
...
func (lc *LightningChannel) validateFeeRate(feePerKw dcrutil.Amount) error {
	...
	newFee := lnwire.NewMSatFromSatoshis(
		dcrutil.Amount((int64(feePerKw) * txWeight) / 1000),
	)
...
// the remote.
func (lc *LightningChannel) UpdateFee(feePerKw dcrutil.Amount) error {
	lc.Lock()
	defer lc.Unlock()
...
func (lc *LightningChannel) ReceiveUpdateFee(feePerKw dcrutil.Amount) error {
	lc.Lock()
	defer lc.Unlock()
...
// units of sat-per-kw.
func (lc *LightningChannel) CommitFeeRate() dcrutil.Amount {
	lc.RLock()
	defer lc.RUnlock()
